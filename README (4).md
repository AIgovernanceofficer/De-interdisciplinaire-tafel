...
De hele app is één HTML-bestand zonder server of installatie. Je kunt het lokaal
openen of als statische pagina hosten (bijvoorbeeld op GitHub Pages).
...
- Gebruik je een eigen API-sleutel (zie hieronder), dan gaat het verkeer
  **rechtstreeks** van jouw browser naar Anthropic.
...
## Zelf hosten (bijv. GitHub Pages)

1. Plaats het HTML-bestand in een (publieke) GitHub-repository.
2. Zet GitHub Pages aan in de repository-instellingen.
3. Open de pagina, klap **"Geavanceerd"** open en plak je **Anthropic API-sleutel**.
   Die blijft in je browser en wordt nergens opgeslagen.

Let op bij gebruik in de klas:

- Een API-sleutel die je in de browser invoert, is in principe zichtbaar voor de
  gebruiker van die browser. Geef studenten daarom niet zomaar een gedeelde sleutel.
- Voor een klas is een **institutionele proxy of gateway** veiliger: studenten
  gebruiken de app dan zonder eigen sleutel, en het sleutelbeheer en de
  toegangscontrole liggen centraal.
- Aan API-gebruik zijn kosten verbonden; meer bijdragen per ronde en langere
  gesprekken kosten meer tokens.
