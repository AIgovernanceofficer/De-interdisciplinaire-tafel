...
2. **Kies de modus.** *AI-debat met Claude* geeft het rijkste, meest inhoudelijke
   gesprek (internet/API nodig). *Offline* werkt zonder internet of API: dan stelt de
   app zelf een eenvoudiger, disciplinegericht debat samen op basis van vaste
   bouwstenen.
3. **Kies hoeveel bijdragen** elke expert per ronde inbrengt (1 = kort en bondig,
   3 = uitgebreid debat).
4. **Voer een dilemma in.** De drie experts reageren om de beurt, spreken elkaar bij
   naam aan en sluiten af met een vraag aan jou.
5. **Reageer** zo vaak je wilt; het gesprek bouwt door.
6. **Exporteer naar Word** om het volledige gesprek als verslag te bewaren of in te
...
- **Claude (Anthropic)** — in de AI-modus genereert het taalmodel `claude-sonnet-4-6`,
  aangeroepen via de Anthropic Messages API, het debat. Dit is het enige onderdeel dat
  een internetverbinding nodig heeft.
- **Offline-debatmotor** — in de offline-modus stelt de app het debat zelf samen met
  ingebouwde, disciplinegerichte tekstbouwstenen. Geen internet, geen API, geen
  gegevens die je browser verlaten — wel een eenvoudiger gesprek dan met Claude.
...
- **Voer geen bijzondere of tot personen herleidbare gegevens in.** Het gebruik valt
  onder de gebruikscondities van AI-tools van de Universiteit Utrecht:
  <https://www.uu.nl/organisatie/ai-beleid/studenten/ai-tools-voor-studenten/gebruikscondities-van-ai-tools>.
